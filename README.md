...
- **截图识别（本机 OCR）**：先判断截图类型（睡眠 / 全天活动 / 单次训练）再解析；只认标签对应的数值，目标值（1,232/2,000 千卡）取实际值，图表坐标与状态栏时间不作数；识别不到就显示「未识别到有效数据」，有歧义标「待确认」
- **一天多次训练**：每次训练一张卡片（项目 / 开始时间 / 时长 / 内容 / 记录方式 / 感受），训练次数按卡片自动统计；每张卡片可以单独上传当次训练的截图，数据不会串到别的训练，也不会与全天活动重复相加

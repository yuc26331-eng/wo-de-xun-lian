...
- **导入历史报告（PDF 归档）**：把以前的 ChatGPT 每日总结 PDF 按报告日期导入成正式记录（训练场次 / Apple Watch / 睡眠 / 补剂 / 饮食 / 报告原文 + 原始 PDF 附件）；导入前自动备份，同一天只补空字段、不覆盖已有数据，冲突逐条列出

## 历史报告导入（`public/import/`）

站点是公开的 GitHub Pages，历史报告里包含训练、睡眠、补剂等健康数据，
因此**明文数据不放进仓库**：`public/import/history.json` 只放元数据（算法、盐、IV、日期范围），
`history.bin` 是 AES-256-GCM 密文（密钥由导入码经 PBKDF2-SHA256 / 250k 次迭代派生）。
用户在自己设备上输入一次导入码，浏览器本地解密后写入 IndexedDB，不上传任何数据。

打包脚本（仓库外，含明文数据）：`work/build_history_bundle.py <导入码> [输出目录]`。
生成端与网站端都会把导入码规范化为「去掉分隔符、统一大写」，因此输入时带不带连字符都可以。
